...
document.addEventListener("file-loaded", async () => {
  ...
  updateExpandBackBtn();
  if (currentFile) {
    if (spectrogramCache.has(currentFile.name)) {
      displayCached(currentFile);
    } else {
      const arrayBuf = await currentFile.arrayBuffer();
      const ac = new (window.AudioContext || window.webkitAudioContext)();
      const audioBuf = await ac.decodeAudioData(arrayBuf.slice(0));
      specWorker.postMessage({ type: "render", key: currentFile.name, buffer: audioBuf.getChannelData(0), sampleRate: audioBuf.sampleRate, fftSize: currentFftSize, overlap: getOverlapPercent() }, [audioBuf.getChannelData(0).buffer]);
    }
    preloadNextFile();
  }
});
