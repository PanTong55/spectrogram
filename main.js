...
import { getSpectrogramImage, preloadFile, getAudioBuffer, preloadNeighbors } from './modules/cacheManager.js';
...
document.addEventListener("file-loaded", async () => {
  ...
  if (currentFile) {
    ...
    if (cachedImg) {
      const clone = await createImageBitmap(cachedImg);
      specWorker.postMessage({ type: "drawImage", image: clone }, [clone]);
    } else {
